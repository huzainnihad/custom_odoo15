...
class BiMaterialTransfer(models.Model):
    _inherit = 'bi.material.transfer'

    @api.model
    def create(self,vals):
        user_lock = self.env['user.lock.date'].search([('lock_date', '=', True)], order='date desc')[0]
        material_transfer_date = vals['confirmation_date']
        mate_date = fields.Date.from_string(material_transfer_date)
        if user_lock.date > mate_date:
            raise UserError(_("your cannot material transfer before user lock date"))
        res = super(BiMaterialTransfer, self).create(vals)
        return res
